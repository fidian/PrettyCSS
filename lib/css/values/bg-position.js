...
var lengthPercent = require('./lengthPercent');
...
var testPatterns = function (patterns, unparsedReal, parser, container) {
	...
	for (var pm in patterns) {
		...
		switch (pm) {
			...
			case 'TB':
				if (unparsed.isContent('top') || unparsed.isContent('bottom')) {
					doesMatch = true;
				}
			...
			case 'PL':
				token = base.lengthOrPositionParse(unparsed, parser, container);
		...
		if (doesMatch) {
			...
			return base.extend({}, patterns[pm], {
				tokens: [ token ],
				unparsed: unparsed
			});
		}
...
exports.parse = function (unparsedReal, parser, container) {
	...
	var childDef = testPatterns(this.patterns, unparsedReal, parser, bp);
