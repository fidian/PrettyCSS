...
exports.parse = function (unparsedReal, parser, container) {
	var bc = new BorderColor(parser, container, unparsedReal);
	...
	var hits = bc.repeatParser([ borderColorSingle ], 4);
	...
	if (hits == 1 && bc.firstToken().content.toLowerCase() == "transparent") {
		bc = new BorderColor(parser, container, unparsedReal);
		bc.debug("reparse as transparent");
		validate.call(bc, 'minimumCss', 2);
