	...
	functionParser: function () {
		...
		while (args.length) {
			if (matchCount > 1) {
				...
				this.unparsed.advance();  // Skip comma and possibly whitespace
			}

			var parsed = this.unparsed.matchAny(args.shift(), this);
		...
		if (! this.unparsed.isTypeContent('PAREN_CLOSE', ')')) {
			return false;
		}

		this.unparsed.advance();  // Skip the close parenthesis
		var myself = this;
	...
	scanRules: function () {
		...
		for (var i = 0; i < rules.length; i ++) {
			...
			for (var j = 0; j < values.length; j ++) {
				var result = this.testRuleValue(values[j], tokenContent, rule);
