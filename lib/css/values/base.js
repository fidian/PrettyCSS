	...
	handleInherit: function (validator) {
		if (this.unparsed.isContent('inherit')) {
			...
			if (typeof validator == 'undefined') {
				validate.call(this, 'minimumCss', 2);
			} else {
				validator(this);
			}
			return true;
	...
	repeatParser: function (possibilities, maxHits) {
		...
		while (keepGoing && unparsed.length()) {
			...
			if (possibilities.some(function (tryMe) {
				// Try the parse() method if it exists
				if (tryMe.parse) {
					var obj = tryMe.parse(unparsed, myself.parser, myself);
				...
				// Should be a string - use isContent()
				if (unparsed.isContent(tryMe)) {
					matches.push(unparsed.advance());
					return true;
				}
