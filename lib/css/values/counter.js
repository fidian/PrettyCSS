...
exports.parse = function (unparsed, bucket, container) {
	...
	if (! result) {
		result = counter.functionParser('counter(', bucket['ident'], bucket['string']);
	}
	
	if (! result) {
		result = counter.functionParser('counter(', bucket['ident'], bucket['string'], bucket['list-style-type']);
	}
