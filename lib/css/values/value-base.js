...
exports.warnIfNotInteger = function (obj, token, value, min, max) {
	// CSS1 lets you use a period after an integer for percentages, but
	// let's not worry about that.
	if (! /^[-+]?[0-9]+$/.test(value)) {
		obj.addWarning('only_integers_allowed', token);
	}
};
...
exports.warnIfOutsideRange = function (obj, token, value, min, max) {
	...
	if (v > max) {
		obj.addWarning('out_of_range_max_' + max, token);
		return min;
	}
	if (v < min) {
		obj.addWarning('out_of_range_min_' + min, token);
		return min;
	}
