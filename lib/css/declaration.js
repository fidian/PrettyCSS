...
var valueColor = require('./values/color');
var valueDisplay = require('./values/display');
...
exports.parse = function (tokens, parser, container) {
	...
	if (result.tokens.isType('S')) {
		result.tokens.shift();
	}

	if (result.tokens.length()) {
		parser.addWarning("extra_tokens_after_value", result.tokens.firstToken());
	}

	declaration.value.setList(result.tokens);
	declaration.value.prepend(result.value);
