...
var Tokenizer = function (options) {
	...
	this.tokens = [];
	this.tokenIndex = 0;
	this.defs = getTokenDefs();
	this.defsByLetter = getDefsByLetter(this.defs);
};
...
Tokenizer.prototype.tokenize = function (str) {
	...
	while (str.length) {
		...
		var type = null;
		var match = '';
		var defs = this.defsByLetter[''];
		var firstLetter = str.charAt(0);

		if (this.defsByLetter[firstLetter]) {
			defs = this.defsByLetter[firstLetter];
		}

		// Find the pattern that matches the best
		for (idx in defs) {
			if (type === null) {
				var matches = str.match(defs[idx].regexp);
